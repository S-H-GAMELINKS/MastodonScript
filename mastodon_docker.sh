...
sudo su -l docker -c "cd ~ && git clone https://github.com/tootsuite/mastodon.git live && cd ~/live && git checkout $(git tag -l | grep -v 'rc[0-9]*$' | sort -V | tail -n 1) && vi docker-compose.yml && cp .env.production.sample .env.production && docker-compose pull && docker-compose build && docker-compose run --rm web bundle exec rake mastodon:setup && docker-compose up -d"
